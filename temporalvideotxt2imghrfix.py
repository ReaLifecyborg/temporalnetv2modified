...
y_paths = get_image_paths("/home/streamline/sd-webui-docker/output/animation/tempv2/refrun")
lineart_input_image_path = get_image_paths("/home/streamline/sd-webui-docker/output/animation/tempv2/lineartrun")
segmentation_input_image_path = get_image_paths("/home/streamline/sd-webui-docker/output/animation/tempv2/segrun")
output_directory = "/home/streamline/sd-webui-docker/output/animation/tempv2/output1"
...
def send_request(last_image_path, optical_flow_path, current_image_path, lineart_input_image_path, segmentation_input_image_path):
    ...
    # Resize the last image to match the size of the flow image
    ...
    with open(lineart_input_image_path, "rb") as b:
        lineart_input_image = base64.b64encode(b.read()).decode("utf-8")

    with open(segmentation_input_image_path, "rb") as b:
...
last_image_path = init_image_path
for i in range(1, len(y_paths)):
    print("Path 1:", y_paths[i - 1])
    print("Path 2:", y_paths[i])
    ...
    result = send_request(last_image_path, optical_flow, y_paths[i], lineart_input_image_path[i], segmentation_input_image_path[i])
    data = json.loads(result)

    for j, encoded_image in enumerate(data["images"]):
        if j == 0:
            output_image_path = os.path.join(output_directory, f"output_image_{i}.png")
            last_image_path = output_image_path
        else:
            output_image_path = os.path.join(output_directory, f"controlnet_image_{j}_{i}.png")
